...
//===----------------------------------------------------------------------===//
// LinxV4 ABI Implementation.
//

namespace {
class LinxV4ABIInfo : public DefaultABIInfo {
private:
  unsigned XLen;
  unsigned FLen;
  static const int NumArgGPRs = 6;
...
public:
  LinxV4ABIInfo(CodeGen::CodeGenTypes &CGT, unsigned XLen, unsigned FLen)
      : DefaultABIInfo(CGT), XLen(XLen), FLen(FLen) {}
...
void LinxV4ABIInfo::computeInfo(CGFunctionInfo &FI) const {
  QualType RetTy = FI.getReturnType();
  if (!getCXXABI().classifyReturnType(FI))
    FI.getReturnInfo() = classifyReturnType(RetTy);
...
// responsible for checking that if there is only a single field then that
// field is a float.
bool LinxV4ABIInfo::detectFPCCEligibleStructHelper(
...
// value, fp+fp, or int+fp of appropriate size). If so, NeededArgFPRs and
// NeededArgGPRs are incremented appropriately.
bool LinxV4ABIInfo::detectFPCCEligibleStruct(QualType Ty, llvm::Type *&Field1Ty,
                                             CharUnits &Field1Off,
                                             llvm::Type *&Field2Ty,
                                             CharUnits &Field2Off,
                                             int &NeededArgGPRs,
...
// Field1Ty, Field1Off, Field2Ty, Field2Off. This method will create an
// appropriate coerceToType and unpaddedCoerceToType.
ABIArgInfo LinxV4ABIInfo::coerceAndExpandFPCCEligibleStruct(
    llvm::Type *Field1Ty, CharUnits Field1Off, llvm::Type *Field2Ty,
    CharUnits Field2Off) const {
  SmallVector<llvm::Type *, 3> CoerceElts;
  SmallVector<llvm::Type *, 2> UnpaddedCoerceElts;
...
ABIArgInfo LinxV4ABIInfo::classifyArgumentType(QualType Ty, bool IsFixed,
                                               int &ArgGPRsLeft,
                                               int &ArgFPRsLeft) const {
  assert(ArgGPRsLeft <= NumArgGPRs && "Arg GPR tracking underflow");
  Ty = useFirstFieldIfTransparentUnion(Ty);
...
ABIArgInfo LinxV4ABIInfo::classifyReturnType(QualType RetTy) const {
  if (RetTy->isVoidType())
    return ABIArgInfo::getIgnore();
...
Address LinxV4ABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
                                 QualType Ty) const {
  CharUnits SlotSize = CharUnits::fromQuantity(XLen / 8);
...
ABIArgInfo LinxV4ABIInfo::extendType(QualType Ty) const {
  int TySize = getContext().getTypeSize(Ty);
  // RV64 ABI requires unsigned 32 bit integers to be sign extended.
  if (XLen == 64 && Ty->isUnsignedIntegerOrEnumerationType() && TySize == 32)
    return ABIArgInfo::getSignExtend(Ty);
...
namespace {
class LinxV4TargetCodeGenInfo : public TargetCodeGenInfo {
public:
  LinxV4TargetCodeGenInfo(CodeGen::CodeGenTypes &CGT, unsigned XLen,
                          unsigned FLen)
      : TargetCodeGenInfo(std::make_unique<LinxV4ABIInfo>(CGT, XLen, FLen)) {}
...
//===----------------------------------------------------------------------===//
// LinxV5 ABI Implementation.
//

namespace {
class LinxV5ABIInfo : public DefaultABIInfo {
private:
  unsigned XLen;
  unsigned FLen;
  static const int NumArgGPRs = 6;
...
public:
  LinxV5ABIInfo(CodeGen::CodeGenTypes &CGT, unsigned XLen, unsigned FLen)
      : DefaultABIInfo(CGT), XLen(XLen), FLen(FLen) {}
...
void LinxV5ABIInfo::computeInfo(CGFunctionInfo &FI) const {
  QualType RetTy = FI.getReturnType();
  if (!getCXXABI().classifyReturnType(FI))
    FI.getReturnInfo() = classifyReturnType(RetTy);
...
// responsible for checking that if there is only a single field then that
// field is a float.
bool LinxV5ABIInfo::detectFPCCEligibleStructHelper(
    QualType Ty, CharUnits CurOff, llvm::Type *&Field1Ty, CharUnits &Field1Off,
    llvm::Type *&Field2Ty, CharUnits &Field2Off) const {
  bool IsInt = Ty->isIntegralOrEnumerationType();
  bool IsFloat = Ty->isRealFloatingType();
...
// value, fp+fp, or int+fp of appropriate size). If so, NeededArgFPRs and
// NeededArgGPRs are incremented appropriately.
bool LinxV5ABIInfo::detectFPCCEligibleStruct(QualType Ty, llvm::Type *&Field1Ty,
                                             CharUnits &Field1Off,
                                             llvm::Type *&Field2Ty,
                                             CharUnits &Field2Off,
                                             int &NeededArgGPRs,
...
// Field1Ty, Field1Off, Field2Ty, Field2Off. This method will create an
// appropriate coerceToType and unpaddedCoerceToType.
ABIArgInfo LinxV5ABIInfo::coerceAndExpandFPCCEligibleStruct(
    llvm::Type *Field1Ty, CharUnits Field1Off, llvm::Type *Field2Ty,
    CharUnits Field2Off) const {
  SmallVector<llvm::Type *, 3> CoerceElts;
  SmallVector<llvm::Type *, 2> UnpaddedCoerceElts;
...
ABIArgInfo LinxV5ABIInfo::classifyArgumentType(QualType Ty, bool IsFixed,
                                               int &ArgGPRsLeft,
                                               int &ArgFPRsLeft) const {
  assert(ArgGPRsLeft <= NumArgGPRs && "Arg GPR tracking underflow");
  Ty = useFirstFieldIfTransparentUnion(Ty);
  ...
  if ((!isAggregateTypeForABI(Ty) && !Ty->isVectorType())|| Ty->isExtVectorType()) {
    // Treat an enum type as its underlying type.
    if (const EnumType *EnumTy = Ty->getAs<EnumType>())
      Ty = EnumTy->getDecl()->getIntegerType();
...
ABIArgInfo LinxV5ABIInfo::classifyReturnType(QualType RetTy) const {
  if (RetTy->isVoidType())
    return ABIArgInfo::getIgnore();
...
Address LinxV5ABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
                                 QualType Ty) const {
  CharUnits SlotSize = CharUnits::fromQuantity(XLen / 8);
...
ABIArgInfo LinxV5ABIInfo::extendType(QualType Ty) const {
  int TySize = getContext().getTypeSize(Ty);
  // RV64 ABI requires unsigned 32 bit integers to be sign extended.
  if (XLen == 64 && Ty->isUnsignedIntegerOrEnumerationType() && TySize == 32)
    return ABIArgInfo::getSignExtend(Ty);
...
namespace {
class LinxV5TargetCodeGenInfo : public TargetCodeGenInfo {
public:
  LinxV5TargetCodeGenInfo(CodeGen::CodeGenTypes &CGT, unsigned XLen,
                          unsigned FLen)
      : TargetCodeGenInfo(std::make_unique<LinxV5ABIInfo>(CGT, XLen, FLen)) {}
...
void LinxV5TargetCodeGenInfo::setTargetAttributes(
    const Decl *D, llvm::GlobalValue *GV, CodeGen::CodeGenModule &CGM) const {
  const FunctionDecl *FD = dyn_cast_or_null<FunctionDecl>(D);
  if (!FD)
    return;
  ...
  auto *Fn = cast<llvm::Function>(GV);
  if (FD->hasAttr<LinxBLKFuncVECAttr>()) {
    Fn->addFnAttr("__vec__");
    Fn->addFnAttr("no-builtins");
    Fn->addFnAttr(llvm::Attribute::NoInline);
  } else if (FD->hasAttr<LinxBLKFuncMTCAttr>()) {
    Fn->addFnAttr("__mtc__");
    Fn->addFnAttr("no-builtins");
    Fn->addFnAttr(llvm::Attribute::NoInline);
  }

  if (FD->hasAttr<LinxBLKFuncNoSpillAttr>()) {
    Fn->addFnAttr("blkv-no-spill");
  }
}
...
const TargetCodeGenInfo &CodeGenModule::getTargetCodeGenInfo() {
  ...
  case llvm::Triple::linx64v4: {
    ...
    unsigned XLen = getTarget().getPointerWidth(0);
    unsigned ABIFLen = 0;
    return SetCGInfo(new LinxV4TargetCodeGenInfo(Types, XLen, 64));
  }
  ...
  case llvm::Triple::linx64v5: {
    ...
    unsigned XLen = getTarget().getPointerWidth(0);
    unsigned ABIFLen = 0;
    return SetCGInfo(new LinxV5TargetCodeGenInfo(Types, XLen, 64));
  }
