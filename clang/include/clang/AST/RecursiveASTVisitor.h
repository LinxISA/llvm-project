...
namespace {
// [BiSheng] Determines whether statement trees are macro
bool isMacro(const Stmt *S) {
  if (!S) {
    return false;
  }
  /// fix Incomplete macro expression, stmt type is NullStmtClass
  if (S->getStmtClass() == clang::Stmt::NullStmtClass) {
    return true;
  }
...
static bool isIdenticalStmt(const ASTContext &Ctx, const Stmt *Stmt1,
  ...
  if (!SkipLoop) {
    if (Expr1 && Expr2) {
      ...
      while (I1 != Expr1->child_end() && I2 != Expr2->child_end()) {
        if (!*I1 || !*I2 ||
            !isIdenticalStmt(Ctx, *I1, *I2, IgnoreSideEffects, SkipLoop))
          return false;
        ++I1;
        ++I2;
      }
  ...
  case Stmt::ImplicitCastExprClass: {
    const ImplicitCastExpr *ICE1 = cast<ImplicitCastExpr>(Stmt1);
    const ImplicitCastExpr *ICE2 = cast<ImplicitCastExpr>(Stmt2);
    if (ICE1->getCastKind() != ICE2->getCastKind())
      return false;
    // not the final three kind, it has children, pass; for example:
    // IntegralToBoolean
    if (ICE1->getCastKind() == CK_LValueToRValue ||
        ICE1->getCastKind() == CK_ArrayToPointerDecay ||
        ICE1->getCastKind() == CK_FunctionToPointerDecay)
      return Expr::isSameComparisonOperand(Expr1, Expr2);
  ...
  case Stmt::CompoundStmtClass: {
    ...
    while (I1 != CompStmt1->body_end() && I2 != CompStmt2->body_end()) {
      if (!isIdenticalStmt(Ctx, *I1, *I2, IgnoreSideEffects, SkipLoop))
        return false;
      ++I1;
      ++I2;
    }
  ...
  case Stmt::UnaryOperatorClass: {
    UnaryOperator *UnaryOp1 =
        const_cast<UnaryOperator *>(cast<UnaryOperator>(Stmt1));
    UnaryOperator *UnaryOp2 =
        const_cast<UnaryOperator *>(cast<UnaryOperator>(Stmt2));
    Expr *UExpr1 = UnaryOp1->getSubExpr()->IgnoreParens();
    Expr *UExpr2 = UnaryOp2->getSubExpr()->IgnoreParens();
    if (!isIdenticalStmt(Ctx, UExpr1, UExpr2, IgnoreSideEffects, SkipLoop))
      return false;
...
enum DiagType {
  ...
  LogicalOperator,
  IdenticalBranches,
  InnerIdenticalConditions,
  PrevIdenticalConditions,
  AlwaysEqual,
  AlwaysTrue,
  AlwaysFalse,
...
using WBrancherMapT =
    std::map<const Stmt *, llvm::SmallVector<const Stmt *, 2>>;

class IdenticalExprVisitor : public RecursiveASTVisitor<IdenticalExprVisitor> {
public:
  ASTContext *Context;
  WBrancherMapT WduplicatedBranchMap;

public:
  // IdenticalExprVisitor only visits nodes
  ...
  virtual bool needIgnore(DiagType DiagID, SourceLocation Loc) = 0;
  virtual void reportIdentical(DiagType DiagID, const Stmt *S = nullptr,
                               const Expr *Op = nullptr,
                               ArrayRef<SourceRange> Sr = None) = 0;
  void checkBitwiseOrLogicalOp(const BinaryOperator *B, bool CheckBitwise);
  void checkComparisonOp(const BinaryOperator *B);
  bool CheckAssignInCond(const Stmt *InStmt, const Stmt *EndStmt);
  void SearchDuplicatedBranchInIfStmt(const IfStmt *I);
};
} // end anonymous namespace

void IdenticalExprVisitor::reportIdenticalExpr(const BinaryOperator *B,
                                               bool CheckBitwise,
                                               ArrayRef<SourceRange> Sr) {
  DiagType DiagID;
  if (CheckBitwise)
    DiagID = BitwiseOperator;
  else
    DiagID = LogicalOperator;
  reportIdentical(DiagID, nullptr, B, Sr);
}

void IdenticalExprVisitor::checkBitwiseOrLogicalOp(const BinaryOperator *B,
                                                   bool CheckBitwise) {
  SourceRange Sr[2];
  ...
  while (const BinaryOperator *B2 = dyn_cast<BinaryOperator>(LHS)) {
    if (B->getOpcode() != B2->getOpcode())
      break;
    if (isIdenticalStmt(*Context, RHS, B2->getRHS())) {
  ...
  if (isIdenticalStmt(*Context, RHS, LHS)) {
    Sr[0] = RHS->getSourceRange();
...
void IdenticalExprVisitor::checkComparisonOp(const BinaryOperator *B) {
  ...
  if (isIdenticalStmt(*Context, B->getLHS(), B->getRHS())) {
    DiagType DiagID;
    if (Op == BO_Cmp)
      DiagID = DiagType::AlwaysEqual;
    else if (((Op == BO_EQ) || (Op == BO_LE) || (Op == BO_GE)))
      DiagID = DiagType::AlwaysTrue;
    else
      DiagID = DiagType::AlwaysFalse;

    reportIdentical(DiagID, nullptr, B);
  }
}

bool IdenticalExprVisitor::CheckAssignInCond(const Stmt *InStmt,
                                             const Stmt *EndStmt) {
  if (!InStmt || !EndStmt)
    return true;
  ...
  case Stmt::CompoundAssignOperatorClass:
    return true;
  case Stmt::UnaryOperatorClass: {
    const UnaryOperator *UnaryOp = cast<UnaryOperator>(InStmt);
    if (UnaryOp->isPostfix() || UnaryOp->isPrefix())
      return true;
    break;
  }
  default:
  ...
  for (const Stmt *SubStmt : InStmt->children()) {
    ...
    if (CheckAssignInCond(SubStmt, EndStmt))
      return true;
...
void IdenticalExprVisitor::SearchDuplicatedBranchInIfStmt(const IfStmt *I) {
  const Stmt *Stmt1 = I->getThen();
  const Stmt *Stmt2 = I->getElse();
  if (!isa<IfStmt>(Stmt2) && !(isMacro(Stmt1) || isMacro(Stmt2))) {
    // Just a simple if with no `else if` branch.
    if (areStatementsIdentical(Stmt1->IgnoreContainers(),
                               Stmt2->IgnoreContainers(), *Context)) {
      reportIdentical(DiagType{IdenticalBranches}, I);
      return;
    }

    this->WduplicatedBranchMap[Stmt1].push_back(Stmt2);
...
IdenticalExprVisitor::VisitBinaryOperator(const BinaryOperator *B) {
  if (needIgnore(IdenticalBranches, B->getExprLoc()))
    return true;
  BinaryOperator::Opcode Op = B->getOpcode();
...
bool __attribute__((used)) IdenticalExprVisitor::VisitIfStmt(const IfStmt *I) {
  const Stmt *Stmt1 = I->getThen();
  const Stmt *Stmt2 = I->getElse();

  bool flag = needIgnore(InnerIdenticalConditions, I->getBeginLoc());
  if (!flag) {
    ...
    if (const CompoundStmt *CS = dyn_cast<CompoundStmt>(Stmt1)) {
      if (!CS->body_empty()) {
        ...
        if (InnerIf &&
            isIdenticalStmt(*Context, I->getCond(), InnerIf->getCond(),
                            /*IgnoreSideEffects=*/false, false)) {
          SourceRange Sr[2];
          Sr[0] = I->getCond()->getSourceRange();
          Sr[1] = InnerIf->getCond()->getSourceRange();
          reportIdentical(InnerIdenticalConditions, nullptr,
                          InnerIf->getCond(), Sr);
        }
      }
    }
  ...
  if (Stmt1 && Stmt2) {
    ...
    while (const IfStmt *I2 = dyn_cast_or_null<IfStmt>(Else)) {
      ...
      if (CheckAssignInCond(const_cast<Stmt *>(Else), Cond2))
        break;

      if (isIdenticalStmt(*Context, Cond1, Cond2, false, false)) {
        SourceRange Sr[2];
        Sr[0] = Cond1->getSourceRange();
        Sr[1] = Cond2->getSourceRange();
        reportIdentical(DiagType{PrevIdenticalConditions}, nullptr, Cond2, Sr);
      }
      Else = I2->getElse();
    }
  }
  ...
  // } else
  //   i = 1;
  SearchDuplicatedBranchInIfStmt(I);
  return true;
}

bool __attribute__((used))
IdenticalExprVisitor::VisitConditionalOperator(const ConditionalOperator *C) {
  if (needIgnore(IdenticalBranches, C->getColonLoc()))
    return true;
  // Check if expressions in conditional expression are identical
  // from a symbolic point of view.

  // We do not try to detect chains of ?: operators.
  if (!isMacro(C->getTrueExpr()) && !isMacro(C->getFalseExpr())) {
    if (areStatementsIdentical(C->getTrueExpr(), C->getFalseExpr(), *Context)) {
      SourceRange Sr[2];
      Sr[0] = C->getTrueExpr()->getSourceRange();
      Sr[1] = C->getFalseExpr()->getSourceRange();
      reportIdentical(DiagType{IdenticalExpressions}, nullptr, C, Sr);
      return true;
    }

    this->WduplicatedBranchMap[C->getTrueExpr()].push_back(C->getFalseExpr());
  }
