// RUN: not %clang %s --target=linx64v5 -S -emit-llvm -O2 -o /dev/null 2>&1 | FileCheck %s
// RUN: not %clang %s --target=linx64v5 -S -emit-llvm -O2 -mlinda-global-var-as-thread-local -o /dev/null 2>&1 | FileCheck %s
