...
// CHECK-NEXT: %2 = tail call <256 x float> @llvm.linx.blk.matmul.v256f32.v256f32.v256f32(i64 16, i64 16, i64 16, i64 1, i64 1, <256 x float> %0, <256 x float> %1)
// CHECK-NEXT: %3 = tail call <256 x float> @llvm.linx.blk.acccvt.v256f32.v256f32(i64 16, i64 16, i64 1, i64 1, i64 27, i64 1, <256 x float> %2)
...
void test(float *pa, float *pb, float *pc) {
  ...
  blk_matmul(M, N, K, TY_FLOAT, TY_FLOAT, C, A, B);
  blk_acccvt(M, N, TY_FLOAT, TY_FLOAT, NZ2ND, true, O, C);
