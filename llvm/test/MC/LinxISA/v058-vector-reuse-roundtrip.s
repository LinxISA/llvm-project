# RUN: llvm-mc -triple=linx64 -filetype=obj %s -o - | llvm-objdump -d --triple=linx64 - | FileCheck %s
	...
	.text
	.globl	test_v058_vector_reuse_roundtrip
	.type	test_v058_vector_reuse_roundtrip,@function
test_v058_vector_reuse_roundtrip:
	BSTART.MSEQ 0
	B.TEXT .body
	B.IOT mask=1111, last, ->t<4KB>
	B.IOT mask=1111, last, ->u<2KB>
	C.B.DIMI 8, ->lb0
	C.B.DIMI 1, ->lb1
	C.BSTART
.body:
	...
	C.BSTOP
	.size	test_v058_vector_reuse_roundtrip, .-test_v058_vector_reuse_roundtrip

# CHECK-LABEL: <test_v058_vector_reuse_roundtrip>:
# CHECK: BSTART.MSEQ
# CHECK: B.TEXT
# CHECK: B.IOT{{[[:space:]]+}}mask=1111, last,{{[[:space:]]+}}->t<4KB>
# CHECK: B.IOT{{[[:space:]]+}}mask=1111, last,{{[[:space:]]+}}->u<2KB>
