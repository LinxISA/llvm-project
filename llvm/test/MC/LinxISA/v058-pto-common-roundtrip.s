...
# RUN: llvm-objdump -d --triple=linx64 %t.o | FileCheck %s --check-prefix=DIS
	...
	B.IOR [zero, sp, a0], ->x3
	B.IOS S1, mask=0001
	B.IOS mask=1111, ->S255<8KB>
	B.IOT t#1, mask=0011, last, ->u<128B>
...
# ENC: B.IOS S1, mask=0001{{.*}}encoding: [0x13,0x90,0x10,0x00]
# ENC: B.IOS mask=1111, ->S255<8KB>{{.*}}encoding: [0x13,0x9e,0xf7,0x0f]
# ENC: B.IOT t#1, mask=0011, last, ->u<128B>{{.*}}encoding: [0x93,0xd2,0x09,0x00]
# ENC: B.IOT t#1, u#2, mask=1111{{.*}}encoding: [0x13,0xc0,0x07,0x44]
# ENC: B.IOT mask=1100, last, ->n<8KB>{{.*}}encoding: [0x93,0x6f,0x0e,0x00]

# DIS: BSTART.VEC{{[[:space:]]+}}TADD, FP16
# DIS: BSTART.SFU{{[[:space:]]+}}TEXP, FP16
# DIS: BSTART.VEC{{[[:space:]]+}}TFMA, FP16
# DIS: B.IOR{{[[:space:]]+}}[zero, sp, a0], ->x3
# DIS: B.IOS{{[[:space:]]+}}S1, mask=0001
# DIS: B.IOS{{[[:space:]]+}}mask=1111, ->S255<8KB>
# DIS: B.IOT{{[[:space:]]+}}t#1, mask=0011, last, ->u<128B>
