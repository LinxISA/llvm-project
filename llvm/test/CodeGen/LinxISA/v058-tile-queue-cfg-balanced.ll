...
entry:
  %tile = call %linx.tile @llvm.linx.tile.tload(ptr %src, i32 6, i32 1, i64 0, i64 8, i64 8, i64 0)
...
join:
  call void @llvm.linx.tile.tstore(ptr %dst, %linx.tile %tile, i32 6, i32 1, i64 0, i64 8, i64 8, i64 0)
  ret void
}
...
; CHECK: BSTART.TLOAD
; CHECK: BSTART.TSTORE
; CHECK: B.IOT{{[[:space:]]+}}t#1, mask=1111, last

define void @net_zero_tile_loop(ptr %src, ptr %dst, i32 %count) {
entry:
  %seed = call %linx.tile @llvm.linx.tile.tload(ptr %src, i32 6, i32 1, i64 0, i64 8, i64 8, i64 0)
  br label %head
...
body:
  %next = call %linx.tile @llvm.linx.tileop.unary(%linx.tile %current, i32 2, i32 6, i32 17)
  %next_index = add i32 %index, 1
  br label %head

exit:
  call void @llvm.linx.tile.tstore(ptr %dst, %linx.tile %current, i32 6, i32 1, i64 0, i64 8, i64 8, i64 0)
  ret void
}
...
; CHECK: BSTART.TLOAD
; CHECK: BSTART.VEC{{[[:space:]]+}}TMUL,
; CHECK: B.IOT{{[[:space:]]+}}t#1, mask=1111, last, ->t<4KB>
; CHECK: BSTART.TSTORE
; CHECK: B.IOT{{[[:space:]]+}}t#1, mask=1111, last
