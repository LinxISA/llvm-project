...
entry:
  ...
  %shared = call i64 asm sideeffect "BSTART.TMOV.L2S.PUBLISH FP32\0AB.IOT $1, mask=1111\0AB.IOS mask=1111, ->${0:S}<256B>", "=S,r"(<1024 x i32> %newest)
  %shared2 = call i64 asm sideeffect "BSTART.TMOV.L2S.PUBLISH FP32\0AB.IOT $1, mask=1111\0AB.IOS mask=1111, ->${0:S}<256B>", "=S,r"(<1024 x i32> %older)
  ret void
}
