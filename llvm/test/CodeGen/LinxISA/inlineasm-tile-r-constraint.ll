...
entry:
  %src0 = call <1024 x i32> asm sideeffect "BSTART.TEXPANDS UINT32\0AB.DATR Null\0AB.IOT last, ->${0:q}<4KB>", "=r"()
  %src1 = call <1024 x i32> asm sideeffect "BSTART.TEXPANDS UINT32\0AB.DATR Null\0AB.IOT last, ->${0:q}<4KB>", "=r"()
  %out = call <1024 x i32> asm sideeffect "BSTART.TADD UINT32\0AB.DATR Null\0AB.IOT $1, $2, last, ->${0:q}<4KB>", "=r,r,r"(<1024 x i32> %src0, <1024 x i32> %src1)
  call void asm sideeffect "# sink $0", "r"(<1024 x i32> %out)
  ret void
}

; CHECK-LABEL: inlineasm_tile_r:
; CHECK: BSTART.TADD UINT32
; CHECK: B.IOT t#{{[1-8]}}, t#{{[1-8]}}, last, ->t<4KB>
