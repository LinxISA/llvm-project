// RUN: clang %s --target=linx64v5 -mcpu=v0.43w -S -O2 -mllvm -enable-branch-hint=true -o - | FileCheck %s --check-prefixes=CHECK-OPEN
// RUN: clang %s --target=linx64v5 -mcpu=v0.43w -S -O2 -mllvm -enable-branch-hint=false -o - | FileCheck %s --check-prefixes=CHECK-CLOSE
