// RUN: clang %s --target=linx64v5 -S -O2 -mllvm -enable-branch-hint=true -o - | FileCheck %s --check-prefixes=CHECK-OPEN
// RUN: clang %s --target=linx64v5 -S -O2 -mllvm -enable-branch-hint=false -o - | FileCheck %s --check-prefixes=CHECK-CLOSE

#define strong_likely(x) __builtin_branch_hint (!!(x), 1)
#define strong_unlikely(x) __builtin_branch_hint (x, 0)

// CHECK-OPEN-NOT:   B.HINT
