; RUN: llc < %s -O2 -enable-all-vector-as-tilereg=true -stop-after=prologepilog -o - | FileCheck %s --check-prefix=PEI
...
; 这个用例故意在 if.then / if.else 两边各自产生一组待 merge 的向量值，
; 再在 if.end 一次性使用它们，把控制流边界附近的寄存器压力拉高。
; 目标不是验证“有没有 spill”本身，而是验证：
; 1. 的确发生了 spill
; 2. 需要在 end_cf 之前完成的 spill，必须留在 Copy2PTerm 之前的 mask 区域，
;    不能被拖到 Copy2PTerm 之后再执行

; PEI-LABEL: name: forced_then_spill
; PEI: stack:
; PEI: type: spill-slot
; PEI-LABEL: bb.2.if.then:
; PEI: SIMT_Continuous_SW_U
; PEI: SIMT_Continuous_SWI_U
; PEI: J %bb.4
; PEI-LABEL: bb.4.if.end:
; PEI: liveins:
; PEI: SIMT_Continuous_SWI_U
; PEI: SIMT_PSEL
; PEI: LinxV5PseudoCopy2PTerm
; PEI-NOT: SIMT_Continuous_SWI_U
; PEI: SIMT_FADD
; PEI: SIMT_Continuous_SW killed
