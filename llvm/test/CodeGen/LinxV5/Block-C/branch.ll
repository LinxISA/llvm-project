...
; RUN: llc < %s -O2 -enable-all-vector-as-tilereg=true -stop-after=virtregrewriter -o - | FileCheck %s --check-prefixes=VRR
...
; TERM-LABEL: name: _Z11test_branchu11matrix_typeILm16ELm16EfES_S_i
; TERM: bb.1.Flow:
; TERM: LinxV5PseudoCopy2PTerm
; TERM: bb.4.if.end:
; TERM: LinxV5PseudoCopy2PTerm
; VRR-LABEL: name: _Z11test_branchu11matrix_typeILm16ELm16EfES_S_i
; VRR-LABEL: bb.2.if.then:
; VRR: %[[THEN:[0-9]+]]:simtcgvl = SIMT_FMUL
; VRR-LABEL: bb.3.if.else:
; VRR: %[[ELSE:[0-9]+]]:simtcgvl = SIMT_Continuous_LW_GLOBAL
; VRR-LABEL: bb.4.if.end:
; VRR: %[[SAVED:[0-9]+]]:simtcgs = LinxV5PseudoCopyFromP
; VRR-NEXT: LinxV5PseudoCopy2PTerm
; VRR-NEXT: {{%[0-9]+}}:simtcgvl = SIMT_PSEL {{.*}} %[[SAVED]], 4, %[[THEN]], 5, %[[ELSE]], 5
; ASM: v.psel{{.*}}vn#1.sw, vn#2.sw, {{.*}}->vt.w
...
; CHECK: %merge = call float @llvm.blkv.merge.cf.f32
; CHECK: call void @llvm.blkv.end.cf.i64
; TERM-LABEL: name: _Z13test_branch_2u11matrix_typeILm16ELm16EfES_S_i
; TERM: bb.1.Flow:
; TERM: LinxV5PseudoCopy2PTerm
; TERM: bb.4.if.end:
; TERM: LinxV5PseudoCopy2PTerm
