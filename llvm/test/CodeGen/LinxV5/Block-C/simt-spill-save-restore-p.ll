; RUN: llc < %s -O2 -enable-all-vector-as-tilereg=true -linxv5-simt-spill-save-restore-p -stop-after=prologepilog -o - | FileCheck %s --check-prefix=PEI
...
; 兜底方案验证：
; 1. PEI 后 spill/reload 仍然正常展开
; 2. spill/reload 前后插入 save/restore p
; 3. spill/reload 执行期间把 p 临时扩成全 1

; PEI-LABEL: bb.4.if.end:
; PEI: $t4 = LinxV5PseudoCopyFromP implicit $simt_p
; PEI-NEXT: $simt_p = SIMT_ORI_SCAR 0, $r0, 0, -1, implicit $simt_p
; PEI-NEXT: SIMT_Continuous_SWI_U $simt_vn3, 1, $simt_to1, 0, 8, implicit $simt_p
; PEI-NEXT: LinxV5PseudoCopy2P $t4, implicit-def $simt_p, implicit $simt_p
; PEI: $t4 = LinxV5PseudoCopyFromP implicit $simt_p
; PEI-NEXT: $simt_p = SIMT_ORI_SCAR 0, $r0, 0, -1, implicit $simt_p
; PEI: SIMT_Continuous_LWI_U 1, $simt_to1, 0, 8, implicit $simt_p
; PEI-NEXT: LinxV5PseudoCopy2P $t4, implicit-def $simt_p, implicit $simt_p

define dso_local void @forced_then_spill(<256 x float> %TA, <256 x float> %TB,
