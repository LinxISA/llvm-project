...
static Register getTileReuseReg(Register Reg) {
...
void SlotCalc::BuildCopy(MachineBasicBlock::iterator Before, LinxRegOp RegOp) {
  ...
  if (TR == LinxV5::NoRegister) {
    if (IsTileRC) {
      // The placeholder uses a zero-mask no-op B.IOT with the minimum legal
      // destination SizeCode (1..10; 0 is the source-only encoding). The
      // encoded capacity preserves the bundle shape without allocating or
      // reading a tile.
...
bool LinxV5TRegToOffsetOpt::runOnMachineFunction(MachineFunction &MF) {
  ...
  if (!EnableReg2Offset) {
    // Preserve the retain decision even in the absolute-register output mode.
    // A bare absolute Tile source now means last-use, so leaving a non-kill
    // operand unmarked would silently release a still-live value.
    for (MachineBasicBlock &MBB : MF)
      for (MachineInstr &MI : MBB)
        for (MachineOperand &MO : MI.operands())
          if (MO.isReg() && MO.isUse() && MO.getReg().isPhysical() &&
              !MO.isKill())
            MO.setReg(getTileReuseReg(MO.getReg()));
    return true;
  }
      ...
      if (!MO.isReg()) {
        ...
        if (isRC(MO.getReg(), RC, MRI)) {
          if (!TRegIdx.count(MO.getReg())) {
            ...
            if (isTileReg(RC) && RC != &LinxV5::TILE_ABS_ACCRegClass &&
                MO.getReg().isPhysical() && !MO.isKill())
              MO.setReg(getTileReuseReg(MO.getReg()));
            continue;
          }
