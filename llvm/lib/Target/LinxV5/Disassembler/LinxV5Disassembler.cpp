...
static DecodeStatus decodeBIOTLifetimeEncoding(MCInst &MI, uint32_t Insn,
  ...
  unsigned TSize = fieldFromInstruction(Insn, 9, 3);
  unsigned DstTile = fieldFromInstruction(Insn, 7, 2);
  bool HasDst = TSize != 0 || DstTile != 0;
  ...
  // Operand order mirrors the TableGen decoder: definitions first, then the
  // PE mask, size, last flag, and sources in encoded order.
  if (HasDst && DecodeTILE_DSTRegisterClass(MI, DstTile, Address, Decoder) ==
                    MCDisassembler::Fail)
    return MCDisassembler::Fail;
  MI.addOperand(MCOperand::createImm(fieldFromInstruction(Insn, 15, 4)));
  if (HasDst)
    MI.addOperand(MCOperand::createImm(TSize));
  MI.addOperand(MCOperand::createImm(fieldFromInstruction(Insn, 19, 1)));
